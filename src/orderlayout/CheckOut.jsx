...
//   // เข้าเพจโดยไม่มี state -> กลับ cart
//   React.useEffect(() => {
//     if (!state?.items?.length) navigate("/cart", { replace: true });
//   }, [state, navigate]);
//   if (!state?.items?.length) return null;
...
//   /* ---------- ชำระเงิน: checkout → confirm → verify ---------- */
//   const [paymentStep, setPaymentStep] = React.useState("idle"); // idle | checking_out | confirming | done
//   const [error, setError] = React.useState(null);
//   const [orderId, setOrderId] = React.useState(null);
...
//   // GET /orders/{order_id} เพื่อตรวจสถานะล่าสุด
...
//       // STEP 2: confirm (จำลองชำระทันที)
//       setPaymentStep("confirming");
//       const confirmRes = await confirmPayment(oid);

//       // ถ้า API ตอบ Paid_Status มาแล้วเป็น Success ก็จบได้เลย
//       const paidNow = (
//         confirmRes?.Paid_Status ||
//         confirmRes?.paid_status ||
//         ""
//       ).toLowerCase();
//       if (paidNow !== "success") {
//         // กรณียัง pending ให้ verify แบบสั้น ๆ 2 รอบ (เผื่อ DB commit ช้า)
//         for (let i = 0; i < 2; i++) {
//           // หน่วงนิด ๆ
//           // eslint-disable-next-line no-await-in-loop
//           await new Promise((r) => setTimeout(r, 250));
//           // eslint-disable-next-line no-await-in-loop
//           const d = await getOrderDetails(oid);
//           const s = (d?.Paid_Status || d?.paid_status || "").toLowerCase();
//           if (s === "success") break;
//           if (i === 1) throw new Error("ยืนยันการชำระเงินไม่สำเร็จ");
//         }
//       }

//       // STEP 3: ล้างตะกร้า + sync + โชว์สำเร็จ
//       clearCart();
//       fetchCart().catch(() => {});
//       setPaymentStep("done");
//     } catch (e) {
...
//               fullWidth
//               sx={{ mt: 3 }}
//               disabled={paymentStep !== "idle" || !address}
//               onClick={handlePlaceOrder}
//             >
//               {paymentStep === "idle" && "PLACE ORDER"}
//               {paymentStep === "checking_out" && "CREATING ORDER..."}
