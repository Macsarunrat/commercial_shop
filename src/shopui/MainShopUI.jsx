...
import { addToCartServer } from "../cart/cartCon";
...
// บังคับ URL ภาพให้ข้ามหน้าเตือน ngrok
function fixNgrok(url) {
  if (!url) return url;
...
// หา cover image ของ product จาก /image/
async function fetchCoverImageByPID(pid, signal) {
  const res = await fetch(`${API}/image/`, { headers: HDRS, signal });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  const arr = await res.json();
  const pidNum = Number(pid);
  const imgs = (Array.isArray(arr) ? arr : []).filter(
    (x) => Number(x.Product_ID ?? x.product_id) === pidNum
  );
  if (!imgs.length) return null;
  const cover = imgs.find((x) => x.IsCover) ?? imgs[0];
  const url = new URL(cover.Img_Src || cover.img_src, API);
...
export default function MainShopUI() {
  ...
  // เลือกรูป detail (state.image > cover จาก /image/ > image จากสินค้า)
  const [detailImage, setDetailImage] = React.useState(null);
  React.useEffect(() => {
    const ctl = new AbortController();
    (async () => {
      if (state?.image) {
        setDetailImage(fixNgrok(state.image));
        return;
      }
      if (!pid) {
        setDetailImage(null);
      ...
      try {
        const url = await fetchCoverImageByPID(pid, ctl.signal);
        setDetailImage(url);
      } catch {
        setDetailImage(null);
      }
  ...
  async function handleAddToCart() {
    ...
    try {
      ...
      await addToCartServer(product.sellId, qty, {
        token: tk,
        useCookie: false, // ถ้าใช้ cookie-session ให้เปลี่ยนเป็น true
      });

      // ดึงตะกร้าจากเซิร์ฟเวอร์ -> อัปเดต badge/ตะกร้าใน state
      await fetchCart({ token: tk, useCookie: false });
  ...
  const { name = "", price = 0, stock = 0, image = "/IMG1/bagG.png" } = product;
  const displayImage = detailImage || fixNgrok(image) || "/IMG1/bagG.png";
